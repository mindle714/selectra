...
class TTSWriter(SummaryWriter):
    ...
    def add_ema(self, p_padded, p_out, mel_lengths, global_step, phase):
        N = p_padded.size(0)
        if N > 10:
            N = 10
        for i in range(N):
            p_fig = plot_ema(p_padded, p_out, mel_lengths, i)    
            self.add_figure(f'pitch/{phase}_{i}', p_fig, global_step)
    ...
    def add_specs_inf(self, mel_padded, mel_out, mel_out_post, mel_lengths, stop_inf, global_step, phase):
        mel_fig = plot_melspec_inf(mel_padded, mel_out, mel_out_post, mel_lengths, stop_inf) 
